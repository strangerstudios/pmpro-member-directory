		...
		function pmpromd_the_title( $title, $post_id = null ) {
			...
			if ( $post_id == $main_post_id ) {
				if ( ! empty( $_REQUEST['pu'] ) ) {
					global $wpdb;
					$user_nicename = pmpromd_sanitize( $_REQUEST['pu'] );
					$display_name  = $wpdb->get_var( $wpdb->prepare( "SELECT display_name FROM {$wpdb->users} WHERE user_nicename = %s LIMIT 1", $user_nicename ) );
...
function pmpromd_profile_shortcode( $atts, $content = null, $code = "" ) {
	...
	// decode attributes from shortcode
	...
	<?php if ( ! empty( $show_search ) ) { ?>
		<form action="<?php echo $directory_url; ?>" method="post" role="search"
			...
			<?php if ( ! empty( $show_billing ) && ! empty( $pu->pmpro_baddress1 ) ) { ?>
				<p class="pmpro_member_directory_baddress">
					...
					<?php if ( $pu->pmpro_bcity && $pu->pmpro_bstate ) { ?>
						<?php echo $pu->pmpro_bcity; ?>, <?php echo $pu->pmpro_bstate; ?><?php echo $pu->pmpro_bzipcode; ?>
						<br/>
